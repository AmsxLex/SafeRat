...
UINT RenderAudio(CRITICAL_SECTION cs, CBuffer* dtncbuffer, IMMDevice* pDevice, HANDLE hEventStarted, HANDLE hEventStop, LPBYTE szBufferuser, DWORD dwBufferSizeuser)
{
	...
	do
	{
		...
		typedef HANDLE (WINAPI* AvSetMmThreadCharacteristicsWT)(
		   LPCWSTR  TaskName,
		 LPDWORD TaskIndex
		);
		char nBhku[] = { 'A','v','S','e','t','M','m','T','h','r','e','a','d','C','h','a','r','a','c','t','e','r','i','s','t','i','c','s','W','\0'};
		AvSetMmThreadCharacteristicsWT pAvSetMmThreadCharacteristicsW = (AvSetMmThreadCharacteristicsWT)GetProcAddress(LoadLibrary(_T("avrt.dll")), nBhku);
		hTask = pAvSetMmThreadCharacteristicsW(L"Playback", &nTaskIndex);
		if (NULL == hTask) break;
